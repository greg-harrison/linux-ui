...
if (xrandr | grep "DP1 disconnected") || (xrandr | grep "HDMI1 disconnected") ; then
  ...
  conky
elif (xrandr | grep "HDMI1 connected") && (xrandr | grep "DP1 disconnected") ; then
  killall conky
  xrandr --output VIRTUAL1 --off --output eDP1 --off --output DP1 --off --output HDMI2 --off --output HDMI1 --primary --mode 1920x1200 --pos 0x0 --rotate normal  --output DP2 --off
  conky
else
  killall conky
