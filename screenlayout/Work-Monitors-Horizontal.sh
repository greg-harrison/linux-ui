...
if (xrandr | grep "HDMI1 disconnected") || (xrandr | grep "HDMI2 disconnected") ; then
  ...
  killall conky
  killall plank
  xrandr --output VIRTUAL1 --off --output eDP1 --off --output DP1 --off --output HDMI2 --primary --mode 1920x1200 --pos 1920x0 --rotate normal --output HDMI1 --mode 1920x1200 --pos 0x0 --rotate normal --output DP2 --off
  conky
  plank
fi
